...
# fix is required and must survive whitespace-collapse as a non-empty string
fixes = st.text(min_size=1, max_size=60).filter(lambda word: word.split())
...
@given(st.lists(terms, max_size=12), st.lists(terms, max_size=8), names, fixes)
...
def test_constructor_rejects_a_bare_string_instead_of_exploding_it():
    ...
    with pytest.raises(TypeError, match="list of terms"):
        Lexicon(name="x", indicates="circle back", fix="ask")
    with pytest.raises(TypeError, match="list of terms"):
        Lexicon(name="x", indicates=["a"], rules_out="b", fix="ask")
...
def test_empty_rules_out_and_evidence_are_omitted_but_fix_is_always_emitted():
    code = Lexicon(name="x", indicates=["a"], fix="do the thing").as_code()
    assert "rules_out" not in code
    assert "evidence" not in code
    # fix is required, so it is never omitted
    assert "fix='do the thing'" in code


def test_a_blank_fix_is_rejected():
    with pytest.raises(AssertionError, match="must have a fix"):
        Lexicon(name="x", indicates=["a"], fix="   ")
