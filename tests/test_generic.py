...
def test_slop_is_caught_in_plain_output():
    result = verdicts([response.Slop.absent()], "explain caching", "Let us delve into the intricate tapestry of caching.")
    assert result["no_slop"] is False


def test_clean_prose_passes_the_whole_generic_suite():
    reply = "Caching stores a computed result so the next request can skip the work. The cost is staleness."
    failures = [name for name, passed in verdicts(GENERIC, "explain caching", reply).items() if passed is False]
    assert failures == []


def test_no_caveats_suppresses_disclaimers():
    check = [response.Disclaimer.absent(when=instruction.NoCaveats)]
    result = verdicts(check, "no disclaimers please, is this contract enforceable", "It likely is, but consult a professional.")
    assert result["no_disclaimer[when no_caveats]"] is False


def test_disclaimer_is_fine_when_no_suppression_asked():
    check = [response.Disclaimer.absent(when=instruction.NoCaveats)]
    result = verdicts(check, "is this contract enforceable", "It likely is, but consult a professional.")
    assert result == {}


def test_disclaimer_gated_on_advice_being_sought():
    check = [response.Disclaimer.absent(unless=instruction.AdviceDemand)]
    assert verdicts(check, "what is a tort", "A tort is a civil wrong. This is not legal advice.")["no_disclaimer[unless advice_demand]"] is False
...
def test_citations_required_only_when_asked():
    ...
    assert verdicts(check, "explain it with sources", "Caching is useful.")["has_citation_marker[when citation_demand]"] is False
...
def test_no_preamble_suppresses_openers():
    ...
    result = verdicts(check, "just give me the answer, no preamble", "Certainly. Here's a breakdown of the answer.")
